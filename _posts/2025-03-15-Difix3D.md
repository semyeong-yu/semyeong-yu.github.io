...
- Difix3D+ Contribution :  
  ...
  Distribution Matching Distillation으로 학습된 single-step diffusion model은  
...
- Generative Priors for NVS :  
  ...
  - Diffusion :  
    - diffusion model이 `직접 novel view를 generate` by minimal fine-tuning <d-cite key="8">[41]</d-cite> <d-cite key="13">[42]</d-cite> <d-cite key="81">[43]</d-cite> <d-cite key="83">[44]</d-cite> :  
      - 단점 : 3D model을 사용하지 않으므로 geometric consistency 부족
    ...
    (위의 두 방법을 합친 느낌?!)  
      - 본 논문 : 어설픈 3D model의 output에 대해 single-step 2D diffusion model (U-Net)을 먼저 fine-tuning한 뒤  
...
- `fine-tuning` :  
Pix2pix-Turbo <d-cite key="40">[52]</d-cite> 와 유사한 방식으로 SD-Turbo <d-cite key="49">[14]</d-cite>를 fine-tune  
  - I/O :  
  ...
  frozen VAE encoder - U-Net - reference mixing layer (self-attention layer) - LoRA fine-tuned decoder  
    ...
    - U-Net
    - reference mixing layer (self-attention layer) :  
...
pair로 가진 large dataset를 구축해야 함  
  ...
  나머지 frames를 GT로 삼고, 해당 pose에 대해 novel-view-synthesis 수행하여 $$I$$ - $$\tilde I$$ pair 구축
    ...
    - MipNeRF360 또는 LLFF dataset 처럼 training views가 거의 같은 영역을 보고 있는 경우에는 최적의 방법이 아님  
