...
- Generative Priors for NVS :  
  ...
  - Diffusion :  
    ...
    - diffusion model이 3D model을 fine-tuning하기 위해 `training image set을 augment` <d-cite key="difix">[11]</d-cite> <d-cite key="27">[50]</d-cite> <d-cite key="28">[51]</d-cite>  
    장점 : `diffusion model을 먼저 fine-tuning`한 뒤  
    3D model을 fine-tuning할 때 diffusion model을 `single-step으로 query`하면 돼서 overhead 감소
