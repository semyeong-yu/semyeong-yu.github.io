...
- rasterize_points.cu :  
  - RasterizeGaussiansCUDA() : color, radii 저장을 위한 tensor 만들고 geometry, bin, image 저장을 위한 buffer 만든 뒤  
...
- rasterizer_impl.cu :  
  ...
  (아래의 ### FORWARD section에 설명)
    - FORWARD::preprocess() 호출  
    $$\rightarrow$$ preprocessCUDA<> <<<>>>() 호출
    - cub::DeviceScan::InclusiveSum() 호출
    - duplicatedWithKeys<<<,>>>() 호출
    - cub::DeviceRadixSort::SortPairs() 호출
    - identifyTileRanges<<<,>>>() 호출
    - FORWARD::render() 호출  
    $$\rightarrow$$ renderCUDA<> <<<>>>() 호출
