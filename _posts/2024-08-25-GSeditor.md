...
- `Edit loss` :  
  ...
  1) `DreamFusion` <d-cite key="Dreamfusion">[1]</d-cite> 의 `SDS loss`처럼 3D model의 noised rendering과 other conditions를 2D diffusion model에 넣어준 뒤, noise 넣고 `denoising하는 과정에서 내놓은 score`가 3D model의 업데이트 방향을 guide  
