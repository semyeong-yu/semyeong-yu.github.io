...
- 기존 NeRF의 문제점 : sampling할 때 single ray per pixel 사용하므로  
blurry or aliased rendering at diff. resolution  
그렇다고 multiple rays per pixel로 brute-force supersampling하는 것은 정확하긴 하겠지만 too costly 비현실적  

- Mip-NeRF : represent scene as `continuously-valued multi-scale`  
