...
- $$T(t) = \exp(- \int_{t_n}^{t} \sigma (r(s)) ds)$$ : transmittance (투과도가 클수록 투명)  
...
Here, in volume rendering, author uses `volume density` within each interval  
as 불투명도 == opacity == extinction coefficient == alpha value for alpha-compositing  
...
If we map input (5D coordinate) into higher dim. space, MLP can learn high-frequency scene representation  
$$r : R \rightarrow R^{2L}$$  
$$p : $$x$$ or $$y$$ or $$z$$ or $$\theta$$ or $$\phi$$  
...
$$C(r) = \sum_{i=1}^{N_c} T_i \alpha_{i} c_i = \sum_{i=1}^{N_c} w_i c_i$$  
...
=> We sample 128 `fine` points following PDF of normalized $$\frac{w_i}{\sum_{j=1}^{N_c} w_j}$$
...
We evaluate `fine` network using $$N_f$$=64+128=192 points per ray where we sample 128 points following PDF of `coarse` sampled points  
In result, we use a total of $$N_c + N_f$$=256 samples per ray to compute the final rendering color $$C(r)$$  
...
1. From H x W input image, choose 1024 pixels
2. calculate continuous ray from each pixel $$r(t) = o + kd$$
3. coarse sampling of $$N_c$$=64 points per each ray $$t_i$$ ~ $$U \left[t_n + \frac{i-1}{N_c}(t_f - t_n), t_n + \frac{i}{N_c}(t_f - t_n)\right]$$
4. positional encoding of $$x, y, z, \theta, \phi$$ for input
5. obtain volume density $$\sigma$$ by MLP with $$x, y, z$$ as input
6. obtain color $$c$$ by MLP with $$x, y, z, \theta, \phi$$ as input
7. obtain rendering color of each ray by volume rendering $$C(r) = \sum_{i=1}^{N} T_i (1 - \exp(- \sigma_{i} \delta_{i})) c_i$$ for two networks 'coarse' and 'fine'
8. compute loss

Here, we use L2 norm for loss  
$$L = \sum_{r\inR} \left[{\left\|\hat{C_c}(r)-C(r)\right\|}^2+{\left\|\hat{C_f}(r)-C(r)\right\|}^2\right]$$  
$$C(r)$$ : GT pixel color  
$$\hat{C_c}(r)$$ : rendering color from coarse network  
$$\hat{C_f}(r)$$ : rendering color from fine network
