...
=> We sample $$N_f$$ `fine` points following PDF of normalized $$\frac{w_i}{\sum_{j=1}^{N_c} w_j}$$
...
We sample $$N_c$$=64 `coarse` points, and then we sample $$N_f$$=128 `fine` points following PDF of `coarse` sampled points  
In result, we use both $$N_c + N_f$$ samples to compute the final rendering color $$C(r)$$  
