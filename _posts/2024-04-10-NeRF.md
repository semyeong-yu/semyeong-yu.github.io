...
5. obtain volume density $$\sigma$$ by MLP with $$r(x, y, z)$$ as input
6. obtain color $$c$$ by MLP with $$r(x, y, z)$$ and $$r(\theta, \phi)$$ as input
7. obtain rendering color of each ray by volume rendering $$C(r) = \sum_{i=1}^{N} T_i (1 - \exp(- \sigma_{i} \delta_{i})) c_i$$ from two networks 'coarse' and 'fine'
8. compute loss
