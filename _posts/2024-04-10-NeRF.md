...
`transmittance가 클수록 투명해서 no color에 가까우니까 색상 채널의 값이 작게 나타나야 하는 것 아니야?????`
...
`transmittance`  
transmittance = 투과도 = `CDF` that a ray `does not hit` any particles from $$z_0$$ to $$z$$  
투과도가 클수록 투명 (no color)  
...
[How NDC Works?](https://yconquesty.github.io/blog/ml/nerf/nerf_ndc.html#analysis)  
...
Here, `fourier features` (sinusoidal signal은 input signal을 orthogonal space에서 표현 가능) let MLP learn high-frequency function in low-dim. domain <d-cite key="interplation">[1]</d-cite>  
