...
> idea : 빛이 들어올 확률이 클수록, 물체의 밀도가 높을수록 2D 이미지 픽셀에서 색상 채널의 값이 크게 나타나므로  
...
Since solid particles do not overlap for $$\Delta z \rightarrow 0$$, $$A \cdot \Delta z \cdot \rho (z) \cdot \phi r^2$$만큼의 area is occluded  
즉, cross section $$A$$의 $$\frac{A \cdot \Delta z \cdot \rho (z) \cdot \phi r^2}{A} = \phi r^2 \cdot \rho (z) \cdot \Delta z$$만큼 occluded  

If $$\frac{A \cdot \Delta z \cdot \rho (z) \cdot \phi r^2}{A}$$만큼 rays are occluded, the light intensity decreases as  
$$I(z + \Delta z) = (1 - \phi r^2 \rho (z) \Delta z) \prod I(z)$$  
Then the light density difference $$\Delta I = I(z + \Delta z) - I(z) = - \phi r^2 \rho (z) \Delta z \cdot I(z)$$  
즉, $$dI(z) = - \phi r^2 \rho (z) I(z) dz$ = - \sigma (z) I(z) dz$$ where `volume density (or opacity)` is $$\sigma(z) = \phi r^2 \rho (z)$$  
