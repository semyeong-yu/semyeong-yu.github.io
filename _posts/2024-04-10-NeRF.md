...
Then PDF $$p_R(ray) = P[R = c(z)] = p_{hit}(z) = $$  
Then the color of a pixel is expected radiance for ray bounded from $$t_n$$ to $$t_f$$  
$$C(ray) = E[R] = \int_{t_n}^{t_f} R p_R dz = \int_{t_n}^{t_f} c p_{hit} dz = \int_{t_n}^{t_f} T(z) \sigma (z) c(z) dz$$  
...
- $$T(t) = \exp(- \int_{t_n}^{t} \sigma (r(s)) ds) ~~ \rightarrow ~~ T_i = \exp(- \sum_{j=1}^{i-1} \sigma_{j} \delta_{j})$$  
