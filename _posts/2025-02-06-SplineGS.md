...
$$\mu(t) = S(t, \boldsymbol P)$$  
  - input :  
    - time $$t$$
    - a set of $$N_{c}$$ learnable control points $$\boldsymbol P = \{ \boldsymbol p_{k} | \boldsymbol p_{k} \in R^{3} \}_{k \in [0, N_{c}-1]}$$
  ...
  $$S(t, \boldsymbol P) = (2t_{r}^{3} - 3t_{r}^{2} + 1) \boldsymbol p_{\lfloor t_{s} \rfloor} + (t_{r}^{3} - 2t_{r}^{2} + t_{r}) \boldsymbol m_{\lfloor t_{s} \rfloor} + (-2t_{r}^{3} + 3t_{r}^{2}) \boldsymbol p_{\lfloor t_{s} \rfloor + 1} + (t_{r}^{3} - t_{r}^{2}) \boldsymbol m_{\lfloor t_{s} \rfloor + 1}$$  
    - $$t_{n} = t \frac{N_{c} - 1}{N_{f} - 1}$$ : TBD  
    (optimal $$N_{c}$$ can be estimated by MACP)
    - $$t_{r} = t_{s} - \lfloor t_{s} \rfloor$$ : TBD
    - $$\boldsymbol m_{t} = (\boldsymbol p_{t+1} - \boldsymbol p_{t-1})/2$$ : approx. tangent(기울기) of control point $$\boldsymbol p_{t}$$
