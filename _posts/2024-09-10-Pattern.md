...
- discriminant function :  
  ...
  $$g = \boldsymbol a^{T} \boldsymbol y$$  
    ...
    $$\rightarrow$$  
    $$J(\boldsymbol a) = \text{lim}_{n \rightarrow \infty} \frac{1}{n} J_{s}(\boldsymbol a) = \cdots = P(w_1) E_{1}[(\boldsymbol a^{T} \boldsymbol y - 1)^{2}] + P(w_2) E_{2}[(\boldsymbol a^{T} \boldsymbol y + 1)^{2}] = \cdots = \int (\boldsymbol a^{T} \boldsymbol y - g_{opt}(\boldsymbol x))^{2} p(\boldsymbol x) dx + 1 - \int g_{opt}^{2}(\boldsymbol x) p(\boldsymbol x) dx = \epsilon^{2} + 1 - \int g_{opt}^{2}(\boldsymbol x) p(\boldsymbol x) dx$$  
...
- decision rule :  
  ...
  - margin :  
  margin $$M = \text{argmin}_{\boldsymbol x \in D} d(\boldsymbol x) = \text{argmin}_{\boldsymbol x in D} \frac{| \boldsymbol w^{T} \boldsymbol x + b |}{\| \boldsymbol w \|}$$  
  where margin은 support vectors(extreme vectors)가 결정
  - goal :  
  $$\text{argmax}_{\boldsymbol w, b} M = \text{argmax}_{\boldsymbol w, b} \text{argmin}_{\boldsymbol x \in D} d(\boldsymbol x) = \text{argmax}_{\boldsymbol w, b} \text{argmin}_{\boldsymbol x \in D} \frac{| \boldsymbol w^{T} \boldsymbol x + b |}{\| \boldsymbol w \|}$$  
    - maximize $$M$$ s.t. $$y_{i} (\boldsymbol w_{i}^{T} \boldsymbol x_{i} + w_{0})$$
