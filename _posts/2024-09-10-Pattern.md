...
($$\sigma_{1} \geq \sigma_{2} \geq \cdots \geq \sigma_{r} \geq \sigma_{r+1} = \cdots = \sigma_{m} = 0$$)  
  ...
  - $$\text{null}(A^T)$$ is spanned by $$u_{r+1}, \cdots, u_m$$  
  $$AA^Tu_i = 0$$ for $$i = r+1, \cdots, m$$, 즉 $$A^T u_i = 0$$
  - $$\text{row}(A) = \text{col}(A^T)$$ is spanned by $$v_{1}^{T}, \cdots, v_{r}^{T}$$  
  $$A^TAv_i = \sigma_{i}^{2} v_i$$ for $$i = 1, \cdots, r$$
  - $$\text{null}(A)$$ is spanned by $$v_{r+1}^{T}, \cdots, v_{n}^{T}$$  
...
- SVD 계산 과정 Summary :  
  ...
  $$V^TV=I$$ 맞는지 확인 
  - Step 3)  
  ($$AA^T$$ 를 Eigen Decomposition해도 되긴 하지만)  
