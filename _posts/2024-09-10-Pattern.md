...
- parametric approach :  
$$N(\mu, \Sigma)$$ 등 parametric approach는  
$$\mu, \Sigma$$ 라는 parameter를 모를 뿐  
Normal Distribution이라는 pdf 형태는 안다는 걸 전제로 한다  
하지만 실제로는 어떤 pdf 형태를 가지는지 모를 때가 많다
  - 방법 1)  
  Non-parametric approach로 samples로부터 prior pdf를 직접 estimate
  - 방법 2)  
  아예 posterior를 직접 estimate  
  e.g. Direct Decision Rule - Nearest-Neighbor Rule
...
But, pdf 모를 때는?
  ...
  - $$\text{lim}_{n \rightarrow \infty} \hat p(x) = p(x)$$ 위해  
    ...
    - e.g. $$k = \sqrt{n}$$  
    so that $$\text{lim}_{n \rightarrow \infty} k = \infty$$ and $$\text{lim}_{n \rightarrow \infty} \frac{k}{n} = 0$$ 
...
$$\phi (u) = \begin{cases} 1 & \text{if} & | u_i | \leq \frac{1}{2} \\ 0 & \text{O.W.} \end{cases}$$ for $$i = 1, \ldots, d$$  
  - indicator function처럼 쓰여서 cube $$(h_n)^d$$ 범위 안에 들어오는 sample 개수 $$k$$ 세는 데 사용  
...
### Non-parametric Estimation for Classification
