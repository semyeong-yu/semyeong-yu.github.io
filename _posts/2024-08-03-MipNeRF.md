...
conical frustum의 모양과 크기에 대한 parameter가 주어진다면 frustum 내부의 점 $$x$$ 에 sin 및 cos을 씌운 $$\gamma (x)$$ 의 경우 $$x$$ 에 대해 공간 적분할 수 있지 않나요?
- A3 : 
...
$$w_k^{\ast} = \frac{1}{2}(max(w_{k-1}, w_k) + max(w_k, w_{k+1})) + \alpha$$
- A5 : 
- Q6 : frustum을 multi-variate Gaussian으로 근사하기 위해서는 먼저 $$F(x, o, d, \hat r, t_0, t_1)$$ 의 mean, covariance를 계산해야 한다고 하는데  
appendix를 보면 F의 mean과 covariance가 아니라 conical frustum의 모양과 크기에 대한 parameter를 가지고 공간 적분해서 t, x, y 축의 mean과 variance를 계산하지 않나요?
- A6 : 
