...
> 핵심 요약 :  
1. ray-tracing하여 point-encoding 대신 cone-tracing하여 region-encoding 이므로 frustum의 모양과 크기 정보를 encode할 수 있어서 scale 반영 가능  
2. IPE 단계에서 `high variance (distant view)`일 때 `high freq.를 attenuate` (pre-filtering) 하여  
...
- Integrated Positional Encoding (IPE) :  
  ...
  Since $$E_{k \sim N(\mu, \sigma^2)}[e^{itk}] = exp(i \mu t - \frac{1}{2} \sigma^2 t^2)$$ and $$sin(k) = \frac{e^{ik}-e^{-ik}}{2i}$$,  
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-08-03-MipNeRF/7.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
- IPE vs PE :  
  ...
  $$\rightarrow$$ `high-freq.` PE features are `aliased` (PE over interval oscillates repeatedly)  
  - IPE :  
  interval region을 integrate하여 encode  
  IPE feature를 만드는 데 사용된 interval $$t \in [t_0, t_1]$$ width보다 period가 작은 `high freq.`의 경우 encoding을 `attenuate`하여 `anti-aliasing`  
...
이게 Mip-NeRF의 핵심!!  
  ...
  위와 같이 scale을 반영하므로  
  distant views에 대해서 `anti-aliasing` 가능!  
...
$$(x, y, z) = \phi (r, t, \theta) = t \cdot (r cos \theta , r sin \theta , 1)$$  
where $$\theta \in [0, 2 \pi)$$ and $$t \geq 0$$ and $$\| r \| \leq \hat r$$  
$$\rightarrow$$  
$$dx dy dz = \| det(D \phi) \| dr dt d\theta$$  
...
- Thus the probability density function for points uniformly sampled from the conical frustum is rt2/V . The first moment of t is: 정리할 차례
...
- 아래 내용들을 제외하고는 NeRF의 Architecture와 동일  
  ...
  $$\rightarrow$$  
  interval(region)-encoding 이므로 $$n$$개의 구간에 대해 $$n+1$$개의 point sampling  
  ...
  2-tap max filter와 2-tap blur filter를 사용해서 weight $$w$$ 를 아래와 같이 filtering 하여 weight $$w$$ 의 wide and smooth upper envelope (상계)을 사용  
...
- A3 : frustum 내에 있는 모든 좌표에 $$\gamma$$ 를 씌워서 공간 적분하는 것 자체가 말도 안 되게 복잡한 식이라 closed-form solution이 없기 때문에 frustum의 mean과 variance를 구해서 expected value 구함
- Q4 : $$\Sigma = \sigma_{t}^2(dd^T) + \sigma_{r}^2(I-\frac{dd^T}{\| d \|^2})$$ 의 수식으로 t-coordinate에서 world-coordinate으로 변환할 수 있다는데, ray 방향의 eigen-matrix가 $$dd^T$$ 인 건 이해했는데 왜 ray에 수직인 방향의 eigen-matrix가 $$I-\frac{dd^T}{\| d \|^2}$$ 인가요?
- A4 :
- Q5 : 2-tap max filter와 2-tap blur filter를 사용해서 weight $$w$$ 를 아래와 같이 filtering 하여 $$w$$ 의 wide and smooth upper envelope을 사용했다는데, filter가 특별한 게 아니라 단순히 max 취한 게 max filter 이고 2개 평균 낸 게 blur filter 인가요?  
$$w_k^{\ast} = \frac{1}{2}(max(w_{k-1}, w_k) + max(w_k, w_{k+1})) + \alpha$$
- A5 : 
- Q6 : 논문을 보면 frustum을 multi-variate Gaussian으로 근사하기 위해서는 먼저 $$F(x, o, d, \hat r, t_0, t_1)$$ 의 mean, covariance를 계산해야 한다고 쓰여있던데  
...
- A6 : 맞습니다. 논문에서 $$F(x, o, d, \hat r, t_0, t_1)$$ 의 mean, covariance를 계산해야 한다고 언급되어 있는 것은 단순히 frustum 내부 범위에 속해있는 지점에 대해 적분을 통해 mean, variance를 구해야 한다는 뜻인 것 같습니다.
