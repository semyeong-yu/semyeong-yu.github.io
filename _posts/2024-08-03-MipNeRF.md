...
$$\rightarrow$$ `conical-frustum을 multi-variate Gaussian으로 근사`한 뒤  
...
- frustum을 multi-variate Gaussian으로 근사 :  
  ...
  $$\sigma_{t}^2 = \frac{t_{\sigma}^2}{3} - $$  
  $$\sigma_{r}^2 = \hat r^2 (\frac{t_{\mu}^2}{4} + )$$
  - 위의 param.를 가지는 Gaussian은 `t-coordinate`에서 정의했는데  
  아래 수식에 의해 `world-coordinate`으로 변환할 수 있다  
  수식수식

- Integrated Positional Encoding (IPE) :  
Next, we derive the IPE, which is the expectation of a positionally-encoded coordinate 읽을 차례
