...
> 핵심 요약 :  
1. Mip-NeRF는 training하는 동안, `임의의 scale`에 대해 query 받을 수 있는, scene의 `anti-aliased (pre-filtered)` representation을 학습한다.
2. camera center로부터 각 pixel로 3D conical frustum을 쏜 다음, `frustum을 multi-variate Gaussian으로 근사`한 뒤, Gaussian 내 좌표를 positional encoding한 것에 대해 `integral` $$E \left[ \gamma (x) \right]$$ 계산  
...
`blurry` rendering in `close-up` views (물체 in `high resolution`)  
`aliased`(계단) rendering in `distant` views (물체 in `low resolution`)  
...
두 camera가 같은 3D point를 서로 다른 scale로 바라보고 있을 경우  
두 camera가 동일한 point-sampled feature를 얻지만  
실제로는 camera에는 서로 다른 장면을 포착하고 있음  
이러한 ambiguity가 기존 NeRF의 성능 하락의 요인

- Mip-NeRF :  
volume 정보를 반영하기 위해 camera center로부터 각 pixel로 3D conical frustum을 쏜 다음, 3D point 및 그 주위의 Gaussian region을 encode하기 위해 IPE  
IPE (`integrated positional encoding`) : `frustum을 multi-variate Gaussian으로 근사`한 뒤, Gaussian 내 좌표를 positional encoding한 것에 대해 `integral` $$E \left[ \gamma (x) \right]$$ 계산  
...
input scene의 `geometry를 미리 알 수 없으므로` pre-filtering 할 수가 없다  
...
- cone per pixel :  
  ...
  so that image plane에서의 cone의 $$x, y$$ variance가 pixel's footprint의 variance와 같아지도록 `?????`  
  - $$t \in [t_0, t_1]$$ 일 때 conical frustum 내의 $$x$$는 아래에 해당되는 값을 가질 수 있다  
  수식수식수식수식
