...
B. The L Hyperparameter in PE and IPE 읽을 차례 `?????`
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-08-03-MipNeRF/11.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
position은 Gaussian으로 근사해서 IPE 씀  
  - x : mean of shape (B, S, 3) and y : var of shape (B, S, 3, 3)  
  - x_enc : $$Px$$ 의 mean, 즉 PE-basis-lifted mean of shape (B, S, $$L-l$$ * 6)  
  where 6 = 3 (3d-vector) * 2 (sin and cos)  
  $$\mu_{r} = P \mu = \begin{bmatrix} \mu_{x} & \mu_{y} & \mu{z} \\ \vdots & \vdots & \vdots \\ \mu_{x} & \mu_{y} & \mu{z} \end{bmatrix} \circledast \begin{bmatrix} 2^l & 2^l & 2^l \\ \vdots & \vdots & \vdots \\ 2^{L-1} & 2^{L-1} & 2^{L-1} \end{bmatrix}$$ .reshape(B, S, -1) = $$[\mu_{x}2^l, \mu_{y}2^l, \mu_{z}2^l, \ldots, \mu_{x}2^{L-1}, \mu_{y}2^{L-1}, \mu_{z}2^{L-1}]$$ of shape ($$L-l$$ * 3,)  
  where $$\circledast$$ is element-wise multiplication  
  $$\cos{Px} = \sin{Px + \frac{\pi}{2}}$$
  - y_enc : $$Px$$ 의 var, 즉 PE-basis-lifted var of shape ()  
  `???`
...
- origins, directions, radii : shape (B, 3)
- t_vals : shape (B, N_samples+1)
