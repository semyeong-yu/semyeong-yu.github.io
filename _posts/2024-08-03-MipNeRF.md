...
> 핵심 요약 :  
1. ray-tracing하여 point-encoding 대신 cone-tracing하여 region-encoding  
...
- IPE vs PE :  
  ...
  - high freq.는 attenuate하므로 `L을 hyper-param.로 두지 않고 extremely large fixed-value`로 두면 된다  
...
F(x, .)의 mean, covariance를 먼저 계산해야 한다 `?????`
...
- 아래 내용들을 제외하고는 NeRF의 Architecture와 동일  
  ...
  대신 IPE feature 자체가 scale을 반영할 수 있으므로 MLP 하나만으로 multi-scale representation 학습  

Mip-NeRF therefore
uses a single MLP with parameters Θ, which we repeatedly
query in a hierarchical sampling strategy. This has multiple
benefits: model size is cut in half, renderings are more accurate,
sampling is more efficient, and the overall algorithm
