...
- 아래 내용들을 제외하고는 NeRF의 Architecture와 동일  
  ...
  대신 interval(region)-encoding 이므로 $$n$$개의 구간에 대해 $$n+1$$개의 point sampling  
  ...
  대신 IPE feature 자체가 scale을 반영할 수 있으므로 MLP 하나를 반복해서 써서 hierarchical sampling  
...
conical frustum의 모양과 크기에 대한 parameter가 주어진다면 frustum 내부의 점 $$x$$ 에 대해 공간 적분할 수 있지 않나요?
- A3 : 
