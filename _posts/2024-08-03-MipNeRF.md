...
- frustum을 multi-variate Gaussian으로 근사하기 위해서는  
먼저 $$F(x, o, d, \hat r, t_0, t_1)$$ 의 mean, covariance를 계산해야 한다 `????`
...
- 아래 내용들을 제외하고는 NeRF의 Architecture와 동일  
  ...
  2-tap max filter와 2-tap blur filter를 사용해서 weight $$w$$ 를 아래와 같이 filtering 하여 $$w$$ 의 wide and smooth upper envelope을 사용 `?????`  
...
- Q4 : $$\Sigma = \sigma_{t}^2(dd^T) + \sigma_{r}^2(I-\frac{dd^T}{\| d \|^2})$$ 의 수식으로 t-coordinate에서 world-coordinate으로 변환할 수 있다는데 해당 수식 유도 이해하지 못했습니다
- A4 :
- Q5 : 2-tap max filter와 2-tap blur filter를 사용해서 weight $$w$$ 를 아래와 같이 filtering 하여 $$w$$ 의 wide and smooth upper envelope을 사용했다는데 이해가 가지 않습니다  
$$w_k^{\ast} = \frac{1}{2}(max(w_{k-1}, w_k) + max(w_k, w_{k+1})) + \alpha$$
- A5 : 
