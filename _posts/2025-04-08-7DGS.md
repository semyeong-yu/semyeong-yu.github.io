...
- Q1 :  
$$t, d$$ 가 바뀜에 따라 7DGS의 param.를 Adaptive Gaussian Refinement로 변형하는 건 합리적이라고 생각합니다.  
또한, 7DGS를 3DGS로 slice할 때 $$t, d$$ 에 따라 opacity(contribution)가 달라지는 것도 합리적이라고 생각합니다.  
그런데 7DGS를 3DGS로 slice할 때 3D spatial mean $$\mu_{cond}$$ 이 $$t, d$$ 에 따라 달라지는 것도 합리적인가요?
