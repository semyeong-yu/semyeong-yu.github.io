...
- custom CUDA kernel :  
<d-cite key="Point1">[3]</d-cite> 의 방법을 확장하여  
tile-based rasterization을 위해  
custom CUDA kernel를 추가하여 사용 like <d-cite key="Plenoxels">[1]</d-cite>, <d-cite key="superfast">[9]</d-cite>
...
- Comparison :  
  ...
  - Traning/Rendering `Speed` : NeRF 계열 중 SOTA인 `InstantNGP` <d-cite key="InstantNGP">[2]</d-cite>, `Plenoxels` <d-cite key="Plenoxels">[1]</d-cite> 과 비교  
    - <d-cite key="InstantNGP">[2]</d-cite> , <d-cite key="Plenoxels">[1]</d-cite> 과 비슷한 quality 가질 때까지 training 5-10 min.밖에 안 걸리고,  
...
- Comparison :  
  ...
  complex shape with a lower number of param.을 모델링하는 데도 쓰일 수 있음  
    ...
    - <d-cite key="Point3">[12]</d-cite> 의 point cloud의 4분의 1만큼만 써도 작은 model size로도 <d-cite key="Point3">[12]</d-cite> 의 PSNR 넘겨버림  
