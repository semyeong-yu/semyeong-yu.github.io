...
- `Guard Band` :  
image plane에 가까우면서 view frustum에서 멀리 떨어진 3D Gaussians의 경우 `?????`  
...
- `Remove` :  
$$\alpha$$ 값이 작은 경우 3D Gaussians 제거  
...
$$\rightarrow$$ view-space positional gradient $$\Delta_{p} L$$가 큼 (scene을 제대로 표현하기 위해선 Gaussian position을 크게 옮겨야 함)  
...
`over-reconstruction`의 경우 3D Gaussians split  
  - split : 1개의 Gaussian을 `2개로 분리`하고 각 scale을 줄인 후 초기 Gaussian 확률밀도값에 따라 배치  
  - 조건 1. `view-space positional gradient` $$\Delta_{p} L$$의 avg. magnitude가 큼  
  - 조건 2. `covariance`가 큼  
...
- 3000 iter.마다 $$\alpha$$ 값을 `주기적으로 0으로 초기화` 하면 전체 Gaussian 조절에 큰 도움이 됨!  
  - 효과 1. volumetric 기법의 특성상 `camera와 가까운 영역에 많은 floater들`이 생겨서 Gaussian이 무작위로 증가하게 되는데, 이를 제거해주는 역할  
