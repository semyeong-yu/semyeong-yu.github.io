...
some operations에 대해 custom CUDA kernel를 추가하여 사용 like <d-cite key="Plenoxels">[1]</d-cite>, <d-cite key="superfast">[9]</d-cite>
...
- `Backward process` :  
  - backward process를 위해 <d-cite key="Point1">[3]</d-cite>처럼 global memory에 blended points list를 저장할 수도 있지만 global memory에 부하가 올 수도 있어서 tile마다 구했던 range 및 sorted Gaussian list를 재사용  
...
- Efficient Rasterization :  
  ...
  an entire image에 대해 가장 작은 원소(`primitives`)를 미리 정렬(`pre-sort`)하여 `?????`  
...
`over-reconstruction`의 경우 3D Gaussians split  
  ...
  - 조건 1. `view-space positional gradient` $$\Delta_{p} L$$의 avg. magnitude가 큼 above threshold $$\tau_{pos}$$  
  - 조건 2. `covariance`가 큼  
