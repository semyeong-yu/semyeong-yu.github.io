...
some operations에 대해 custom CUDA kernel를 추가하여 사용 like <d-cite key="Plenoxels">[1]</d-cite>, <d-cite key="superfast">[10]</d-cite>
...
아래의 경우 projected 2D covariance 계산이 불안정하기 때문에 개별적으로 제거  
  - `view frustum near plane에 가까이 있는` Gaussian의 경우,  
...
- 3000 iter.마다 $$\alpha$$ `알파 값을 주기적으로 0으로 초기화` 하면 전체 Gaussian 조절에 큰 도움이 됨!  
  - 효과 1. volumetric 기법의 특성상 `camera와 가까운 영역`에서 Gaussian density가 증가하여 많은 `floater`들이 생기는데, 이를 제거해주는 역할  
  floater 해결 관련 논문 : <d-cite key="floater1">[7]</d-cite> <d-cite key="floater2">[8]</d-cite> <d-cite key="floater3">[9]</d-cite>  
