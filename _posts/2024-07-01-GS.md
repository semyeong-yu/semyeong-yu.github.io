...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/5.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/6.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/7.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/8.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/9.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/10.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
- `Guard Band` :  
image plane에 가까우면서 view frustum에서 멀리 떨어진 3D Gaussians의 경우 $$\leftarrow$$ `?????`  
projected 2D covariance 계산이 불안정하기 때문에 개별적으로 제거  
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/11.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
- `Duplicate with Keys` :  
  - tile마다 각 Gaussian의 key를 생성  
  - CUDA 병렬처리 덕분에 2D Gaussian 하나가 3개의 tiles에 걸쳐 있다면, 3개의 2D Gaussians로 복제(instance화)되는 것처럼 작동

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/12.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
- $$\alpha$$-`Blending` in Order :  
  - tile별 CUDA 병렬처리에 의해 각 pixel에 대해 `color` 및 `opacity` $$\alpha$$ 값을 Gaussian list의 앞에서 뒤로 accumulate  
  `?????` NeRF의 단점이 stochastic sampling해서 accumulate하느라 inefficient하다는 거였는데 얘는 Gaussians 어떻게 더함??  
  - i-th tile에 있는 pixels 중 a pixel opacity 값이 target saturation threshold를 넘어서면, 해당 i-th thread STOP  
  - `Gaussian의 개수를 제한하지 않음`으로써 hyper-param. tuning 없이 arbitrary scene 커버 가능  
...
- `Backward process` :  
  - backward process를 위해 <d-cite key="Pointbased">[5]</d-cite>처럼 global memory에 blended points list를 저장할 수도 있지만 global memory에 부하가 올 수도 있어서 tile마다 구했던 range 및 sorted Gaussian list를 재사용  
  - 직접 backward gradient update 식을 구해서 이용  
  - 각 tile의 Gaussian list에 대해 Gaussian의 `opacity 비율에 따라` 뒤에서 앞으로 gradient update  
  - backward process할 때 accumulated opacity를  
  각 Gaussian opacity로 반복적으로 나누기 때문에  
  gradient 계산을 위한 중간 opacity 값을 구성  
  `?????`
...
- Efficient Rasterization :  
  - Pulsar 논문<d-cite key="Pulsar">[6]</d-cite>에 언급되었듯이  
  가장 작은 원소(`primitives`)를 미리 정렬(`pre-sort`)하는 기법 사용  
  `?????`  
  - differentiable  
  - 2D projection 가능  
  - runtime : pixel마다 O(1)
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/13.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
- 3D Gaussian의 xyz-mean에 대해서만 standard exponential decay scheduling <d-cite key="Plenoxels">[1]</d-cite> 사용
...
- optimization 세부 사항 :  
  ...
  목적 : scene의 corner를 촬영하거나 inside-out 방식(카메라가 촬영 대상의 내부에 위치하여 바깥쪽을 촬영) 때문에 `놓친 angular 영역이 있을 경우 SH의 0-band coeff.가 부적절`하게 만들어질 수 있어서  
  0-band coeff.를 optimize하고 매 1000 iter.마다 band 수 늘려서 4-band coeff.까지 optimization
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/2024-07-01-GS/14.png" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
