...
forward, backward processing할 때  
data loading, sharing을 병렬처리  
...
- $$\alpha$$-`Blending` in Order :  
  - 각 pixel별로 color 및 $$\alpha$$ 값을 Gaussian list의 앞에서 뒤로 accumulate  
  - pixel opacity 값이 target saturation 기준값을 넘어서면, 해당 thread STOP  

- `Backward process` :  
Gaussian의 opacity 비율에 따라 gradient propagation  
`?????`
