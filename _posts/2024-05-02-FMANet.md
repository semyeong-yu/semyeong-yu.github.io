...
globally adaptive to spatio-temporally-variant degradation  
$$k^{D, i} \in R^{H \times W \times C}$$ : degradation features adjusted by conv. from $$K^{D}$$ (motion-aware spatio-temporally-variant degradation kernels) 에 대해  
$$Q=W_{q} k^{D, i}$$  
...
- step 4. FGDF to predict blurry center frame $$\hat X_{c}$$  
$$\hat X_{c}$$ = $$W(\boldsymbol Y, s (\boldsymbol f^{Y} \uparrow))$$ $$\circledast K^{D} \downarrow _{s}$$  
where  
$$\uparrow$$ : $$\times s$$ bilinear upsampling  
$$W(\boldsymbol Y, s (\boldsymbol f^{Y} \uparrow))$$ : warped sharp HR sequence based on an upsampled image flow-mask pair  
...
$$\rightarrow$$ obtain center frame $$\boldsymbol Y_{c}$$ from blurry LR counterpart $$\boldsymbol X$$  
...
normalization : w.r.t all kernels at temporally co-located positions over X `뭔 말인지 코드 한 번 보자`  

- step 3-4. high-frequency detail $$\hat Y_{r}$$  
$$F_{w}^{M} \rightarrow$$ stacked conv. and pixel shuffle $$\rightarrow \hat Y_{r}$$  

- step 4. FGDF to predict sharp center frame $$\hat Y_{c}$$  
...
- loss 1. $$\hat X_{c}$$ $$\leftrightarrow$$ $$X_{c}$$  
- loss 2. <d-cite key="Raft">[16]</d-cite> 
