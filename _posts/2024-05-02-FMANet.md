...
`predict flow and degradation in sharp HR sequence` $$\boldsymbol Y$$  
...
- step 3-1. `an` image flow-mask pair $$\boldsymbol f^{Y} \in R^{T \times H \times W \times (2+1) \dot 1}$$  
$$\boldsymbol f^{M} \rightarrow$$ Conv3d $$\rightarrow \boldsymbol f^{Y}$$  

- step 3-2. $$\hat X_{sharp}$$ only used in Temporal Anchor (TA) loss  
$$F^{M} \rightarrow$$ Conv3d $$\rightarrow \hat X_{sharp} \in R^{T \times H \times W \times 3}$$ in image domain  
...
- step 4. FGDF  
$$\hat X_{c}$$ = $$W(\boldsymbol Y, s \dot (\boldsymbol f^{Y} \uparrow))$$ $$\circledast K^{D}$$  
where  
$$\uparrow$$ : $$\times s$$ bilinear upsampling  
$$W(\boldsymbol Y, s \dot (\boldsymbol f^{Y} \uparrow))$$ : warped sharp HR sequence based on an image flow-mask pair  
$$\circledast K^{D}$$ : FGDF with filter $$K^{D}$$ with stride $$s$$  


> Restoration Network $$Net^{R}$$  
`predict flow and restoration from blurry LR sequence` $$\boldsymbol X$$  
input : blurry LR sequence $$\boldsymbol X$$ and $$F^{M}, \boldsymbol f^{M}, K^{D}$$ from $$Net^{D}$$  
goal : `?????`  

- step 1. It first computes ... 할 차례
