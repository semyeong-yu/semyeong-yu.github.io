...
Boltzmann Machine with bi-partite graph of visible and hidden units  
  ...
  training dataset의 distribution을 학습!!  
    - 훈련 끝나고나면 해당 distribution을 따르는 new sample을 generate할 수 있음!!
    - label과의 joint distribution을 학습하여 classification 수행 가능!!
...
- Training RBM 수식 유도 :  
  ...
  $$\frac{\partial - \text{ln} P(v | \theta)}{\partial \theta} = \cdots = \sum_{h} P(h | v, \theta) \frac{\partial E(v, h | \theta)}{\partial \theta} - \sum_{v, h} P(v, h | \theta) \frac{\partial E(v, h | \theta)}{\partial \theta} = E_{h \sim P(h | v, \theta)}[\frac{\partial E(v, h | \theta)}{\partial \theta}] - E_{(v, h) \sim P(v, h | \theta)}[\frac{\partial E(v, h | \theta)}{\partial \theta}]$$  
    - 오른쪽 expectation 식 :  
    model distribution $$P(v, h | \theta)$$ 모든 경우의 수에 대해 expectation 계산하려면 $$2^{m+n}$$ 로 costly하므로  
    MCMC (Markov chain Monte Carlo) 기법으로 해당 distribution에서 `sampling` 수행하여  
    오른쪽 expectation 식을 approx.
  ...
  proof는 아래 사진에  
    - $$P(h_{i} = 1 | v) = \sigma (\sum_{j=1}^{m} w_{ij} v_{j} + c_{i}) = \sigma (\boldsymbol w_{i} \boldsymbol v + c_{i})$$
    - $$P(v_{j} = 1 | h) = \sigma (\sum_{i=1}^{n} w_{ij} h_{i} + b_{j}) = \sigma (\boldsymbol w_{j}^{T} \boldsymbol h + b_{j})$$
...
- Training RBM 수식 유도 (continued) :  
  ...
  weight $$W, b, c$$ 를 학습  
    - KL-divergence $$D_{KL} (Q(v), P(v | \theta)) = \sum_{v} Q(v) \text{log} \frac{Q(v)}{P(v | \theta)}$$ 를 minimize하면  
    $$\Delta w_{ij} = - \eta \frac{\partial D_{KL}}{\partial w_{ij}} = \eta \sum_{v} Q(v) \frac{\partial - \text{ln} P(v | \theta)}{\partial w_{ij}}$$  
    이렇듯 $$\Delta w_{ij}$$ 는 $$\frac{\partial - \text{ln} P(v | \theta)}{\partial w_{ij}}$$ 에 비례  
    - Gradient of Log-Likelihood :  
    proof는 아래에  
    Let's define $$\boldsymbol h (\boldsymbol v_{0}) = P(\boldsymbol h = \boldsymbol 1 | \boldsymbol v_{0}, \theta) = \sigma(\boldsymbol W \boldsymbol v + \boldsymbol b)$$  
      - $$\nabla_{W} - \text{ln} P(v) = \boldsymbol h (\boldsymbol v_{0}) \boldsymbol v_{0}^{T} - \boldsymbol h (\boldsymbol v_{k}) \boldsymbol v_{k}^{T}$$  
      - $$\nabla_{b} - \text{ln} P(v) = \boldsymbol v_{0}^{T} - \boldsymbol v_{k}^{T}$$  
      - $$\nabla_{c} - \text{ln} P(v) = \boldsymbol h (\boldsymbol v_{0}) - \boldsymbol h (\boldsymbol v_{k})$$
