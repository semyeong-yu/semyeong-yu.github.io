...
- Jackknife :  
  ...
  - jackknife estimate of bias ($$b_{jack}(\hat \theta)$$) :  
  $$\hat \text{bias}_{jack}(\hat \theta) = (n-1)(\hat \theta_{(\cdot)} - \hat \theta)$$  
  where $$\hat \theta_{(\cdot)} = \frac{1}{n} \sum_{i=1}^{n} \hat \theta_{(i)}$$
  - bias-corrected jackknife estimate :  
  $$\hat \theta_{jack} = \hat \theta - \hat \text{bias}_{jack}(\hat \theta) = n \hat \theta - (n-1) \hat \theta_{(\cdot)} = \frac{1}{n} \sum_{i=1}^{n} \tilde \theta_{i}$$  
