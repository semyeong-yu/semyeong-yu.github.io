...
- Training RBM 수식 유도 (continued) :  
  ...
  weight $$W, b, c$$ 를 학습  
    ...
    $$\boldsymbol v_{0}$$ : clamped training sample  
    ...
    KL-divergence $$D_{KL} (Q(v), P(v | \theta)) = \sum_{v} Q(v) \text{log} \frac{Q(v)}{P(v | \theta)}$$ 를 minimize하려면  
      - $$\Delta W \propto \boldsymbol h (\boldsymbol v_{0}) \boldsymbol v_{0}^{T} - \boldsymbol h (\boldsymbol v_{k}) \boldsymbol v_{k}^{T}$$  
      - $$\Delta b \propto \boldsymbol v_{0}^{T} - \boldsymbol v_{k}^{T}$$  
      - $$\Delta c \propto \boldsymbol h (\boldsymbol v_{0}) - \boldsymbol h (\boldsymbol v_{k})$$
...
즉,  
sample $$h_{t}$$ based on $$P(h | v_{t})$$  
and  
sample $$v_{t+1}$$ based on $$P(v | h_{t})$$  
를 반복  
