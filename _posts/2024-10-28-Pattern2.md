...
- Training RBM 수식 유도 (continued) :  
  ...
  weight $$W, b, c$$ 를 학습  
    ...
    KL-divergence $$D_{KL} (Q(v), P(v | \theta)) = \sum_{v} Q(v) \text{log} \frac{Q(v)}{P(v | \theta)}$$ 를 minimize하려면  
      - $$\Delta W \approx \boldsymbol h (\boldsymbol v_{0}) \boldsymbol v_{0}^{T} - \boldsymbol h (\boldsymbol v_{k}) \boldsymbol v_{k}^{T}$$  
      - $$\Delta b \approx \boldsymbol v_{0}^{T} - \boldsymbol v_{k}^{T}$$  
      - $$\Delta c \approx \boldsymbol h (\boldsymbol v_{0}) - \boldsymbol h (\boldsymbol v_{k})$$
