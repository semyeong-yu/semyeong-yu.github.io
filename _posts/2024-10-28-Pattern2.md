...
where there is no connection b.w. visible units or b.w. hidden units
  - probability $$P(v, h | \theta) = \frac{e^{- E(v, h | \theta)}}{Z}$$  
  where energy $$E(v, h | \theta) = - h^{T} W v - b^{T} v - c^{T} h$$ for $$T=1$$
  - goal :  
