...
- `world coordinate` :  
$$\Sigma$$ : covariance matrix of 3D Gaussian  
...
Here, covariance matrix는 positive semi-definite이어야만 physical meaning을 가지는데, $$\Sigma$$ 를 직접 바로 optimize하면 invalid covariance matrix가 될 수 있음  
$$\rightarrow$$ $$\Sigma$$ 가 `positive semi-definite`이도록  
$$\Sigma = R S S^T R^T$$ 로 정의해서  
`scale`을 나타내는 `3D vector` $$s$$ 와 `rotation`을 나타내는 `quaternion` $$q$$ 를 optimize 하자!!  
