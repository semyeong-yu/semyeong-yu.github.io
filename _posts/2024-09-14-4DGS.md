...
- 3DGS를 각 frame에 적용하는 게 아니라 4DGS라는 새로운 모델 제시  
  ...
  - 4DGS framework :  
    ...
    4D neural voxel을 이용해서 Gaussian point-clouds (pts)의 spatial-temporal 정보를 encode  
...
- Spatial-Temporal Structure Encoder (2) :  
  ...
  $$f_{h} = \cup_{l} \prod \text{interp}(R_{l}(i, j))$$  
    ...
    - [Github Code](https://github.com/hustvl/4DGaussians/blob/master/scene/hexplane.py) 에서  
      - forward()  
      - get_density()  
      self.grids : nn.ModuleList() of init_grid_param  
      and  
      init_grid_param() : range(in_dim) = [0, 1, 2, 3] (x, y, z, t) 중에 grid_nd = 2개(plane)를 뽑아서  
      grid_coefs $$R_{l}(i, j) for \{ i, j \} \in \{ (x, y), (x, z), (y, z), (x, t), (y, t), (z, t) \}$$ 반환  
      grid_coefs : nn.ParameterList() of nn.Parameter $$R_{l}(i, j)$$ with shape $$(1, D_{out}, resol. of second dimension, resol. of first dimension)$$  
      e.g. $$R_{l}(x, t)$$, 즉 $$XT$$ plane은 nn.Parameter $$(1, D_{out}, resol.[3], resol.[0])$$ 으로 표현  
      ...
      second argument(from pts) 좌표에서 값을 구하기 위해 first argument(from $$R_{l}(i, j)$$)의 값을 interpolate  
      그럼 이제 3D scene을 `4D neural voxel` 대신 `2D voxel plane` $$R_{l}(i, j)$$ 이라는 param.들로 표현 가능
