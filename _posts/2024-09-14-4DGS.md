...
- Multi-head Gaussian Deformation Decoder :  
  ...
  - [Github Code](https://github.com/hustvl/4DGaussians/blob/master/scene/deformation.py) 에서  
    - Class deformation_network()의 forward_dynamic()
    - Class Deformation()의 forward_dynamic()  
      ...
      - position :  
      $$\gamma(\chi) \times \text{mask} + \Delta \chi$$
      - scaling :  
      $$\gamma(s) \times \text{mask} + \Delta s$$
      - rotation :  
      $$\gamma(r) + \Delta r$$  
      또는  
      quaternion product of $$\gamma(r)$$ and $$\Delta r$$  
      - opacity, SH 도 deform 가능하게 짜놓긴 함  
      $$\alpha \times \text{mask} + \Delta \alpha$$  
      $$k \times \text{mask} + \Delta k$$
