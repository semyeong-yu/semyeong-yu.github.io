...
contract 함수도 consistently `여전히 disparity에 비례`하게 bounded space로 mapping
...
아래 두 가지 loss로 각 MLP를 jointly train  
  ...
  - `proposal loss` :  
    ...
    - 문제 :  
    하나의 histogram bin의 distribution에 대해 아무 것도 가정할 수가 없음  
    ...
    - 위의 가정에 따라  
    NeRF-MLP ($$t$$, $$w$$)의 구간 $$T$$ 와 겹치는 모든 proposal-MLP의 weight $$\hat w_{j}$$ 를 더해서 아래와 같이 $$w$$ 의 `upper bound`를 구하자  
...
- Artifacts :  
  - Floater :  
  가까이 있는 지점에서 불필요한 opacity, color를 예측하여 반투명하게
  - Background Collapse :  
  먼 지점이 가까이 있는 얼룩처럼  
  예상 원인 : disparity에 비례하게 sampling하여 먼 거리의 samples는 덜 sampling하고 이를 bounded space로 mapping하므로 먼 거리의 samples에 대해 왜곡이 일어남

- Artifacts 해결 :  
  - 기존 NeRF :  
  sampling할 때 perturb 인자로 randomness를 추가하여 임의로 정한 일정 간격의 부정확한 지점이 아니라 다양한 정확한 지점에서 학습하도록
  - Mip-NeRF 360 :  
  물체가 있을만한 정확한 지점에서 집중적으로 예측하여 부정확한 지점에서의 불필요한 예측을 억제
...
- A1 : 아래 사진에서처럼 충분히 optimize되어 만약 coarse proposal-MLP가 이미 scene content가 있는 곳을 성공적으로 예측하고 있다면 이를 이용한 fine NeRF-MLP의 fine-samples는 그 곳에 더 촘촘히 존재할 것이므로 bin 간격이 달라져서 두 histogram이 크게 달라보인다  
