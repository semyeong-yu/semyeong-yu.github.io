...
- Define smooth coordinate-transformation function as $$f(x) : R^3 \rightarrow R^3$$  
  ...
  - 이는 state transition model $$f = $$ contract $$(x) = \begin{cases} x & if \| x \| \leq 1 \\ (2 - \frac{1}{\| x \|})(\frac{x}{\| x \|}) & if \| x \| \gt 1 \end{cases}$$ 에 대해  
...
- scene parametrization :  
  ...
  - Mip-NeRF 360 :  
  $$\gamma ($$ contract $$(\mu, \Sigma))$$  
  where contract $$(x) = \begin{cases} x & if \| x \| \leq 1 \\ (2 - \frac{1}{\| x \|})(\frac{x}{\| x \|}) & if \| x \| \gt 1 \end{cases}$$  
...
<div class="caption">
    contract 함수는 파란색 구(radius 1) 외부의 Gaussian(회색)을 주황색 구(radius 2) 내부의 Gaussian(빨간색)으로 mapping
</div>

- ray interval을 disparity (distance의 역수)에 비례하도록 정했다면  
contract 함수도 마찬가지라서  
scene origin에서 cast된 ray의 경우  
contract 함수를 적용한 후에 위의 주황색 영역에서 일정한 길이의 interval을 가진다
