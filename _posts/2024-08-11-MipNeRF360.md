...
proposal-MLP와 NeRF-MLP  
  ...
  Mip-NeRF보다 조금만 더 costly하지만 훨씬 더 higher capacity를 가진 것과 같은 효과  
  $$\rightarrow$$ large unbounded scene을 표현하기에 적절  
...
- Loss :  
  ...
  - `proposal loss` :  
    ...
    둘의 차이인 loss는 0 이다
    - NeRF-MLP ($$t$$, $$w$$)의 구간 $$T$$ 와 겹치는 모든 proposal-MLP의 weight $$\hat w_{j}$$ 를 더해서 아래와 같이 upper bound를 구하자  
    ...
    $$\rightarrow$$  
    아래와 같이 proposal loss는 이를 위반하는 경우에 해당한다  
